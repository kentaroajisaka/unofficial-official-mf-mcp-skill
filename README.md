...
## 関連プロジェクト: mf-full-mcp

このスキルは公式MCPの「使い方」を教えるものですが、公式MCPそのものにできないこともあります。

- **証憑（レシート・請求書）の添付** — 2026年8月時点で公式MCPにツールがありません
- **仕訳の削除** — 公式MCPは登録・更新のみです
- **再認証なしでの複数法人同時接続** — beta版は1事業者のみ、alpha版は複数保持できるがトークンが約1時間で切れます

これらに対応した**非公式のMCPサーバー**を別途公開しています。自前OAuth（動的クライアント登録）で複数法人分のトークンを保持し、自動リフレッシュするため、一度認証すればしばらく放置できます。

→ **https://github.com/kentaroajisaka/mf-full-mcp**

公式MCPで足りている場合は、このスキル＋公式MCPの組み合わせで十分です。
